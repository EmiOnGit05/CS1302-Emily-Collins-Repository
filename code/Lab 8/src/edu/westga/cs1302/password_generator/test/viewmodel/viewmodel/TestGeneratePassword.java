...
class TestGeneratePassword {
	...
	void testMinimumLengthNotANumber() {
		...
		assertEquals("", vm.getPasswords().getValue(), "checking the password property");
	...
	void testValidInputProvided() {
		...
		assertTrue(vm.getPasswords().getValue().get(0).length() >= 2, "checking the password property has an appropriate number of characters");
